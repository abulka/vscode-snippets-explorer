...
async function enumerateSnippets(snippetTree, languageId) {
    ...
    for (const dir of await getThirdPartyExtensionDirs()) {
        ...
        promises.push(listSnippets(snippetsDir, '\.json$', SnippetKind.EXTENSION, languageId, snippetTree))
    }
    ...
    // Look for a data subdirectory underneath vscode install location, just in case Visual Studio Code is used in portable mode - see https://github.com/abulka/vscode-snippets-explorer/issues/9 
    // const vscodeInstallPath = vscode.env.appRoot;
    // console.log('vscodeInstallPath', vscodeInstallPath)  // e.g. '/Applications/Visual Studio Code.app/Contents/Resources/app'
    // let upDir = process.platform == 'darwin' ? '../../..' : '.'  // on mac have to go a bit higher to get out of app bundle
    // let thumbDir = path.join(vscodeInstallPath, upDir, 'data/user-data/User/snippets/')
    // console.log('upDir', upDir, 'thumbDir', thumbDir)  // e.g. '/Applications/Visual Studio Code.app/Contents/Resources/app'
    // masterPromises.push(listSnippets(thumbDir, '\.json$', SnippetKind.USER, languageId, snippetTree))

    await Promise.all(masterPromises)
    removeOutdatedExtensions(snippetTree, { warn: true });

    appLog(`enumerating snippets for '${languageId}' complete!`);
    appLog(`final snippetTree keys are ${JSON.stringify(Object.keys(snippetTree))}`) // dump the root keys keys
