...
async function enumerateSnippets(snippetTree, languageId) {
	...
	// TODO ccould run all these 4 stages in parallel

	// Project snippets
	for (const workspaceFolder of vscode.workspace.workspaceFolders) {
		let vscodeDir = path.join(workspaceFolder.uri.fsPath, '.vscode')
		await listSnippets(vscodeDir, '\.code-snippets$', SnippetKind.PROJECT, languageId, snippetTree)
	}

	// User Snippets
	await listSnippets(getUserSnippetsDir(), '\.json$', SnippetKind.USER, languageId, snippetTree)
	...
	for (const dir of getBundledAppDefaultExtensionDirs()) {
		let snippetsDir = path.join(dir, 'snippets')
		await listSnippets(snippetsDir, '\.code-snippets$', SnippetKind.BUILTIN, languageId, snippetTree)
	}
