...
async function scanGlobalFiles(dir, snippetFiles, kind, languageId, snippetTree) {
    ...
    const globalNodeName = 'global'; // all languages
...
function gatherScanPromises(snippetFiles, languageId, dir, kind) {
    ...
    for (const f of snippetFiles) {
        ...
        // DEBUG avoid adding the same file twice
        ...
        if ((perfectMatch || extensionPathMatch || projectDirMatch) && !addedFilesCache.has(fullPath)) {
            ...
            appLog(`    ✅ ${languageId} - promise scanFile(${fullPath}, ${snippetKindToNiceName(kind).split(' ')[0]}, ${languageId}) because ${perfectMatch ? 'perfectMatch' : extensionPathMatch ? 'extensionPathMatch' : projectDirMatch ? 'projectDirMatch' : 'unknown'}`);
        }
    }
    return promises;
}
...
function isSnippetFileRelevantToLanguageId(filename, languageId, dir, kind) {
    ...
    // We no longer need to expand into multiple languageIds, as we are only dealing with one languageId at a time
    // as we have another loop that will call this function for each languageId (see enumerateSnippets())
    // But it doesn't hurt.
    let validLanguageIds = expandIntoMultipleLanguageIds(languageId); // <- not needed
    // let validLanguageIds = [languageId];

    const validLanguageFileNames = validLanguageIds.map(l => `${l}.json`)
    if (debug) {
        appLog(`   validLanguageIds: ${validLanguageIds} validLanguageFileNames: ${validLanguageFileNames}`)
    }
    ...
    if (debug) {
        appLog(`   dir: ${dir}`);
        validLanguageIds.forEach(languageId => {
            appLog(`   checking if dir.includes(${languageId}-): ${dir.includes(`${languageId}-`)}`);
        });
        appLog(`   pathContainsLanguageIdFragment: ${pathContainsLanguageIdFragment} allowPathMatching: ${allowPathMatching} extensionPathMatch: ${extensionPathMatch}`)
    }
