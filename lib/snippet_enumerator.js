...
async function enumerateSnippets(snippetTree, languageId) {
    ...
    masterPromises.push(listSnippets(getUserSnippetsDir(), '\.code-snippets$', SnippetKind.GLOBAL_USER, languageId, snippetTree, debugSnippetFileNamesFound));

    // Global User Snippets WITHOUT language, applicable always to any open tab - (found in ~/Library/Application Support/Code/User/snippets/ which is the same location as language specific snippets with .json extensions except that global user snippets have arbitrary user defined file names and the .code-snippets file extension).
