        ...
        for (const [extensionId, snippetBaseNames] of Object.entries(extensionIds)) {
            for (const [basename, metaInfoArray] of Object.entries(snippetBaseNames)) {
                if (metaInfoArray.length > 1) {
                    ...
                    appLog(`snippetTree[${languageId}] length before removal: ${Object.keys(snippetTree[languageId]).length}`);
                    ...
                    appLog(`snippetTree[${languageId}] length after removal: ${Object.keys(snippetTree[languageId]).length}`);
                }
            }
        }
    }
