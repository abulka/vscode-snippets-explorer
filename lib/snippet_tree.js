...
class TreeDataProvider {
  ...
  _myWorkspaceLanguageIdChangeListener(event) {
    ...
    if (!new Set(['scminput', 'plaintext']).has(event.languageId)) {
  ...
  _editorChangeListener(event) {
    if (event.document) {
      ...
      appLog(
        `Switched to language ${languageId}, via editor change event, current snippet languages are: ${Object.keys(
          this.snippetTree
        )}`
      );
      ...
      appLog(`Ignoring changed editor change - event.document is '${event.document}'`)
  }
