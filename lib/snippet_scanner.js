...
function scanFiles(dir, snippetFiles, kind, languageId, snippetTree) {
  ...
  snippetFiles.forEach((f) => {
    ...
    if (
      ...
      let snippetsDict = scanFile(fullPath, kind, languageId); // 'languageId' only used if its a .code-snippets and SnippetKind is PROJECT

      if (!isEmpty(snippetsDict)) {
 ...
 *
 * Returns snippet dictionary or undefined if error
 */
