...
function getBundledAppDefaultExtensionDirs() {
	...
	switch (os.type()) {
		...
		case 'Linux':
			...
			// Dump env to see if anything useful here
			appLog(`guessing but app dir might be based on ${JSON.stringify(process.env)}`)
			
			// Another approach is to look at process.env._ which is
			// "_": "/snap/code/39/usr/share/code/code",
			// remove the trailing basename then add '/resources/app/extensions' to get
			// '/snap/code/39/usr/share/code/resources/app/extensions' which also seems to work
			appLog(`TODO should look in ${process.env._}`)
		    // extensionsPath = process.env.HOME + `/${vscode_subdir}/extensions/`;
		    break;
		case 'Windows_NT':
