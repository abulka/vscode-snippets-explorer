...
class TreeDataProvider {
    ...
    async addLanguage(languageId) {
        ...
        if (!keepPreviousLanguages || !Object.keys(this.snippetTree).includes(languageId)) {
            ...
            vscode.window.showInformationMessage(`_rebuildTree 🇨‌🇴‌🇲‌🇵‌🇱‌🇪‌🇹‌🇪‌ Added language '${languageId}' to snippets tree`);
    ...
    async _runJobs(buildJobs) {
        if (buildJobs.length > 1) {
            // Actually we do get here, with "javascript,global" as the buildJobs array, which is a bug - called by refresh button which passes all the keys of the snippet tree, which may inclide 'gloabal' which is not a proper lang id
            appLog(`❌ running multiple jobs per languageId: ${buildJobs}`);
        }
