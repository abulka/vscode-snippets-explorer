	...
	if (debug) {
		...
		for (const [languageId, fullPathSubTree] of Object.entries(snippetTree)) {
			for (const fullPath of Object.keys(fullPathSubTree)) {
				console.log(`Found snippet file ${languageId} - ${fullPath}`)
			}
		}
	}
...
function listSnippets(dir, extension, kind, languageId, snippetTree, debug) {
	...
	if (fs.existsSync(dir)) {
		...
		if (debug) console.log(`${snippetKindToNiceName(kind)} snippet files detected in ${dir} are:\n\t${snippetFiles}`)

		scanFiles(dir, snippetFiles, kind, languageId, snippetTree) // will add entries to the snippetTree
		if (debug) console.log('snippetTree grew to', JSON.stringify(snippetTree))
	}
	else { if (debug) console.log(`no such dir ${dir}`) }
}
...
function getBundledAppDefaultExtensionDirs() {
	...
	switch (os.type()) {
		...
		//     break;
		case 'Windows_NT':
			extensionsPath = `C:\\Program Files (x86)\\Microsoft VS Code\\resources\\app\\extensions\\`; // untested
			break;
		// default:
		//     extensionsPath = process.env.HOME + `/${vscode_subdir}/extensions/`; // ?
		//     break;
