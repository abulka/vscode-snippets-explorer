 ...
 */
function scanFiles(dir, snippetFiles, kind, languageId, snippetTree) {
  // console.log(`DIR ${dir}`) // filled with ${snippetFiles}`)
  console.assert(kind);
  console.assert(languageId);

  snippetFiles.forEach((f) => {
    ...
    if (
      ...
      let scanFileResult = scanFile(fullPath, kind, languageId); // 'languageId' only used if its a .code-snippets and SnippetKind is PROJECT
 ...
 *         happens to need access to again, when processing the async result list
 */
function scanFile(snippetFile, kind, languageId) {
  // Async version has a callback, Sync version does not!
  // fs.readFile(snippetFile, (err, text) => {
  //     console.log('fs.readFileSync', snippetFile)
  //     if (err) {
  //         console.log(`error opening snippet file ${snippetFile} with error ${err}`)
  //         return;
  //     }
  //     // rest of the code
  // }
  ...
  let text;
  try {
    text = fs.readFileSync(snippetFile);
  } catch (error) {
