...
async function enumerateSnippets(snippetTree, languageId) {
    ...
    let languageIds = expandIntoMultipleLanguageIds(languageId);
    appLog(`expanded languageIds for ${languageId} to ${languageIds}`);
    ...
    for (const langId of languageIds) {
        appLog(`Processing language ID: ${langId}`);

        // Project snippets
        for (const workspaceFolder of vscode.workspace.workspaceFolders) {
            let vscodeDir = path.join(workspaceFolder.uri.fsPath, '.vscode')
            masterPromises.push(listSnippets(vscodeDir, '\.code-snippets$', SnippetKind.PROJECT, langId, snippetTree, debugSnippetFileNamesFound))
        }

        // User Snippets
        masterPromises.push(listSnippets(getUserSnippetsDir(), '\.json$', SnippetKind.USER, langId, snippetTree, debugSnippetFileNamesFound))
        ...
        for (const dir of await getThirdPartyExtensionDirs()) {
            ...
            // appLog(`  Scanning extension dir ${dir} promises length ${promises.length}`)
    ...
    appLog(`⏰ Waiting for all promises to resolve for ${languageId}...`);
    await Promise.all(masterPromises).then(() => {
        appLog(`All promises resolved for ${languageId}`);
    }).catch((error) => {
        appLog(`Error resolving promises for ${languageId}: ${error}`);
    });

    removeOutdatedExtensions(snippetTree, { warn: true });

    if (debugSnippetFileNamesFound) {
        // appears in the debug console of the main vscode extension project (not the vscode launched at debug time)
...
async function listSnippets(dir, extension, kind, languageId, snippetTree, debug) {
    ...
    // console.assert(languageId) // languageId can be empty string now
    ...
    const snippetTreeLengthBefore = calcSnippetTreeLength(snippetTree)
    ...
    if (debug) {
        const snippetTreeLengthAfter = calcSnippetTreeLength(snippetTree)
        // weird, snippetTreeLengthBefore never seems to change - probably an async thing?
        if (snippetTreeLengthBefore != snippetTreeLengthAfter) {
            // appLog(`snippetTree grew to ${JSON.stringify(snippetTree)}`)
            appLog(`snippetTree grew to from ${snippetTreeLengthBefore} to ${snippetTreeLengthAfter} ${JSON.stringify(Object.keys(snippetTree))}`)
        }
    }
}

function calcSnippetTreeLength(snippetTree) {
    let result = 0
    for (const [_languageId, fullPathSubTree] of Object.entries(snippetTree)) {
        result += Object.keys(fullPathSubTree).length
    }
    return result
}
...
function getUserSnippetsDir() {
    ...
    if (process.env.WSL_DISTRO_NAME) {
        // When running under WSL, the user snippets are in the windows location!
        // Note in WSL, the process.env.APPDATA environment variable and
        // process.env.USERNAME are not set so use other means to get to the windows home dir and username
        let username = os.userInfo().username;
        const windowsHomePath = `/mnt/c/Users/${username}`;
        settingsPath = path.join(windowsHomePath, 'AppData', 'Roaming', vscode_subdir, 'User/');
    } else {
        switch (os.type()) {
            case 'Darwin':
