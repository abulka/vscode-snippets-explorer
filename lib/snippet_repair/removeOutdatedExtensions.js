  ...
  Reminder, our snippetTree looks like:
    ...
    For each languageId in group
      For each extensionId
        For each basename
          If there are multiple meta entries then we need to pick the latest
          Sort the meta entries by .fullPath and remove the topmost entry, which should be the latest vers
          // The remaining entries are to be killed off from the real snippetTree
...
function removeOutdatedExtensions(snippetTree, options) {
  ...
  // console.log(`grouping is ${JSON.stringify(group.group, null, 2)}`);
      ...
      for (const [basename, metaInfoArray] of Object.entries(snippetBaseNames)) {
        if (metaInfoArray.length > 1) {
          ...
          metaInfoArray.sort(function (a, b) {
            if (a.extensionPathInfo.fullPath == b.extensionPathInfo.fullPath) return 0
            return (a.extensionPathInfo.fullPath < b.extensionPathInfo.fullPath) ? 1 : -1
          });
          ...
          metaInfoArray.forEach(function (metaInfo) { // delete the rest
            ...
            if (options && options.warn)
              console.warn(`Removed out of date snippets for extension ${metaInfo.extensionPathInfo.fullPath}`)
          });
        }
      }
    }
